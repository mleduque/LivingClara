...
END
	...
	+ ~!Global("OHH_HexxatSummoned","GLOBAL",1) !Global("OHH_HexxatSummoned","GLOBAL",2) !InMyArea("hexxat")~ + @9 /*Well, you tried to romance Clara, but she left you for your father.*/ DO ~SetGlobal("YF_LadyDelryn","GLOBAL",1) SetGlobal("SPRITE_IS_DEADHexxat","GLOBAL",1)~ + YF_GoodAnomen3 
...
END
	...
	+ ~!Global("OHH_HexxatSummoned","GLOBAL",1) or(2) !Global("OHH_HexxatSummoned","GLOBAL",2) Dead("Hexxat") !InMyArea("hexxat")~ + @14 /*Yes, with Clara.*/ DO ~SetGlobal("YF_LordAndLadyDelryn","GLOBAL",5) SetGlobal("YF_ClaraRingTOB","GLOBAL",1) SetGlobal("YF_AnomenClaraRomance","GLOBAL",2) SetGlobal("SPRITE_IS_DEADHexxat","GLOBAL",1)~ + YF_BadAnomen3 
	++ @10 /*No, you were not.*/ EXIT
...
END
	++ @21 /*You and Clara professed your love to each other and were married. Your father tried to disown you so we killed him and framed brigands for the murder so that you and Clara became Lord and Lady Delryn. Then we paid a cowled wizard to investigate the murder of Saerk and clear your name. Saerk is officially a villian and we are all officially heroes.*/ DO ~SetGlobal("YF_AnomenIsAntipaladin","GLOBAL",1) ChangeStat("Anomen",CHR,1,ADD) ChangeStat("Anomen",CON,1,ADD) ChangeStat("Anomen",STR,1,ADD) ChangeAlignment("Anomen",LAWFUL_EVIL) ActionOverride("Anomen",AddKit(Blackguard)) ChangeClass("Anomen",PALADIN)~ + YF_BadAnomen4	
